...
` ` `
(코드블럭 안. 요청을 어떻게 이해했는지 자연어 5줄.
 필요하면 ASCII diagram box 사용.)
` ` `

## 💬 think

` ` `
(코드블럭 안. 판단·라우팅·실행 흐름 자연어 5줄.
 필요하면 ASCII diagram box.)
` ` `
...
- 모바일 사용자가 보는 흐름: 굵은 `## 🔧 tools`로 사용 도구 스캔 → `## 🤔 align` 박스로 요청 이해 파악 → `## 💬 think` 박스로 판단 흐름 확인 → 결론(실제 답변)으로 도착
