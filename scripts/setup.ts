...
async function main() {
  ...
  // ── Auth ──────────────────────────────────────────────────────────────────
  header('Auth');
  console.log('  Codex uses ChatGPT OAuth from CODEX_HOME\\auth.json.');
  ...
  const CODEX_HOME = await ask('Codex home', 'C:\\Users\\yeoul\\.codex');
  const CODEX_BIN = await ask('Codex exe path');
